...
if (!function_exists('piratenkleider_setup')):
    function piratenkleider_setup() {
        ...
        /** Entfernen der Wordpressversionsnr im Header FIX:(estavam ambos comentados) */
        ...
        if (!function_exists('get_piratenkleider_socialmediaicons')) :
            ...
            function get_piratenkleider_socialmediaicons($darstellung = 1) {
                ...
                foreach ($default_socialmedia_liste as $entry => $listdata) {
                    ...
                    if (($active == 1) && ($value)) {
                        echo '<li><a class="icon_' . $entry . '" href="' . $value . '" target="_blank">';
      ...
      if ($thisCat->parent != 0) echo(get_category_parents($parentCat, TRUE, ' ' . $delimiter . ' '));
        ...
        function piratenkleider_enqueue_styles() {
            ...
            if ((isset($options['aktiv-alternativestyle'])) && ($options['aktiv-alternativestyle'] != 'style.css')) {
                wp_register_style('aktiv-alternativestyle', get_template_directory_uri() . '/css/' . $options['aktiv-alternativestyle']);
                wp_enqueue_style('aktiv-alternativestyle');
            } else {
                wp_register_style('bloginfo', get_bloginfo('stylesheet_url'));
                wp_enqueue_style('bloginfo');
            }
            if ((isset($options['css-colorfile'])) && (strlen(trim($options['css-colorfile'])) > 1)) {
                wp_register_style('css-colorfile', get_template_directory_uri() . '/css/' . $options['css-colorfile']);
            ...
            if ((isset($options['css-fontfile'])) && (strlen(trim($options['css-fontfile'])) > 1)) {
                wp_register_style('css-fontfile', get_template_directory_uri() . '/css/' . $options['css-fontfile'], array(), false, 'all and (min-width:500px)');
                wp_enqueue_style('css-fontfile');
            }
            if ((isset($options['aktiv-linkicons'])) && ($options['aktiv-linkicons'] == 1)) {
                wp_register_style('src-linkicons-css', $options['src-linkicons-css'], array(), false, 'screen,projection');
                wp_enqueue_style('src-linkicons-css');
            }
            ...
            if (( (isset($custom_fields['fullsize'])) && ($custom_fields['fullsize'][0] == true) && is_singular()) ||
                ...
                wp_register_style('sidebarbottom', get_template_directory_uri() . '/css/basemod_sidebarbottom.css');
                wp_enqueue_style('sidebarbottom');
            }
...
function piratenkleider_header_style() {} 
		...
		wmode: "window", supplied: "' . $supplied . '",';
            ...
            ?>
                                    ...
                                    < div id = "cp_con t ainer_1" cla s s  = "cp-container" >
                                    < div  c lass = "cp-buf fer-holder" ><!-- .cp-g t50 only  n eeded when buffer  is  > than  5 0% -->
                                    < div  clas s = "cp-buffer-1" > < /div> 
                                    < div class = "cp-buffer-2"  > < /div > 
                                    < /div>
                                    < di v class =   "cp-progress-holder"  >< !--.cp - gt50 only needed when progress is > than 50% -->
